...
export default function AdminPanel({ products, onSave, onExit }: AdminPanelProps) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F7F7F7', fontFamily: 'var(--font-sans)' }}>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '32px 24px' }}>
        ...
        {adminView === 'list' && (
          <ProductList products={products} onEdit={handleEdit} onNew={handleNew} />
        )}
        {adminView === 'edit' && (
          <ProductForm
...
function LoginForm({ onLogin, error }: { onLogin: (u: string, p: string) => void; error: string }) {
  ...
  return (
    <div style={{ maxWidth: 360, margin: '60px auto 0' }}>
      ...
      >
        <Field label="Identifiant" id="username">
          <input
            id="username"
            type="text"
            value={username}
        ...
        <button
          type="submit"
          style={{ marginTop: 8, background: '#111', color: '#FFF', border: 'none', padding: '10px 20px', fontFamily: 'var(--font-mono)', fontSize: '0.75rem', letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer' }}
        >
          Se connecter
        </button>
      </form>

      <div style={{ marginTop: 20, fontFamily: 'var(--font-mono)', fontSize: '0.6875rem', color: '#AAA' }}>
        Démo : admin / lab2024
      </div>
    </div>
  )
...
function ProductList({ products, onEdit, onNew }: { products: Product[]; onEdit: (p: Product) => void; onNew: () => void }) {
  return (
    <div>
    ...
    <div style={{ maxWidth: 720 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 10, paddingTop: 8, borderTop: '1px solid #E0E0E0' }}>
          <button
            type="submit"
            style={{ background: '#111', color: '#FFF', border: 'none', padding: '10px 24px', fontFamily: 'var(--font-mono)', fontSize: '0.75rem', letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer' }}
          >
            Enregistrer
          </button>
          <button
