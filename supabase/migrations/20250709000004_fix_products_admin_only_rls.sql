...
drop policy if exists "product_pdfs_public_read" on storage.objects;
create policy "product_pdfs_public_read"
  on storage.objects
  for select
  using (bucket_id = 'product-images');

drop policy if exists "product_pdfs_admin_insert" on storage.objects;
create policy "product_pdfs_admin_insert"
  on storage.objects
  for insert
  to authenticated
  with check (
...
drop policy if exists "product_pdfs_admin_update" on storage.objects;
create policy "product_pdfs_admin_update"
  on storage.objects
  for update
  to authenticated
  using (
...
drop policy if exists "product_pdfs_admin_delete" on storage.objects;
create policy "product_pdfs_admin_delete"
  on storage.objects
  for delete
  to authenticated
  using (
